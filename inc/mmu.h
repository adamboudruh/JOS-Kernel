...
//
// +-------9-------+-------9-------+-------9-------+-------9-------+---------12----------+
// | PML4 Index    |   PDPT Index  |  PD Index     |  PT Index     | Offset within Page  |
// +---------------+---------------+---------------+---------------+---------------------+
//  \- PML4X(la) -/ \-- PDPX(la) -/ \-- PDX(la) --/ \-- PTX(la) --/ \---- PGOFF(la) ----/
//
// The PML4X, PDPX, PDX, PTX, and PGOFF macros decompose linear addresses as shown.
// To construct a linear address la from PML4X(la), PDPX(la), PDX(la), PTX(la), and PGOFF(la),
// use PGADDR(PML4X(la), PDPX(la), PDX(la), PTX(la), PGOFF(la)).
...
// page table level 4 index
#define PML4X(la)		((((uintptr_t) (la)) >> PML4XSHIFT) & 0x1FF)
...
#define PDPXSHIFT	30		// offset of PTX in a linear address
#define PML4XSHIFT	39		// offset of PDX in a linear address

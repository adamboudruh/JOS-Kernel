...
# todo verify if -mcmodel=large is necesary

KERN_CFLAGS := $(CFLAGS) -DJOS_KERNEL -mcmodel=large -m64
BOOT_CFLAGS := $(CFLAGS) -DJOS_KERNEL -m32
USER_CFLAGS := $(CFLAGS) -DJOS_USER -mcmodel=large -m64
...
$(BOOTLOADERDIR)/boot.efi:$(BOOTLOADERDIR)/main.c
	$(V)$(MAKE) -C bootloader
...
QEMUOPTS += -smp $(CPUS)
QEMUOPTS += -bios bios/OVMF.fd -net none
QEMUOPTS += $(QEMUEXTRA)

.PHONY: FORCE

.gdbinit: .gdbinit.tmpl FORCE

	@head -n 6 $< | sed "s/localhost:1234/localhost:$(GDBPORT)/" > $@
